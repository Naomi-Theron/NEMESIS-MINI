...
const events = require('./ridzcoder');
...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [NEMESIS-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'NEMESIS-MD' };
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const myquoted = {
                    key: { remoteJid: 'status@broadcast', participant: '237678687593@s.whatsapp.net', fromMe: false, id: createSerial(16).toUpperCase() },
                    message: { contactMessage: {
                        displayName: '© NEMESIS-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN: NEMESIS-MD\nORG:NEMESIS-MD BOT;\nTEL;type=CELL;type=VOICE;waid=237678687593:237678687593\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© NEMESIS-MD' } }
                    }},
        ...
        arslanLog(`NEMESIS-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 NEMESIS-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
